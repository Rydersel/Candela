...
| `showModelComparison` | Bool | `NO` | Shows the **Model Comparison** section on the Health pane: a soak-only instrument that scores the estimated exposure model (window positions, wallpaper, appearance) against the measured readings. It is kept out of the shipped window until the verdict on that model is recorded, after which the section and this key are removed. The scores accumulate whether or not the section is shown; the key only reveals them. |
