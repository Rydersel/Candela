...
public struct ExposureModelInputs: Sendable {
  ...
  ///
  /// **Front-to-back, and the order is load-bearing.** Under `.topmostWins` it
  /// decides which window owns each cell. Under `.summedCoverage` it changes
...
/// `ExposureModel`.
///
/// `.baseline` is exactly the shipped model and a test pins it: the offline
/// harness replays recorded inputs against the grids the capture tool computed
/// live, and that control is meaningless if the baseline drifts.
public struct ExposureModelParameters: Equatable, Sendable {
  ...
  public enum Compositing: String, Equatable, Sendable, Codable {
    ...
    /// **Cannot express a per-window luminance**, the comparison gate's whole
    /// finding: every fully covered cell gets the same number, so the model can
    /// only vary through the partial-coverage fraction at window edges.
    case summedCoverage
  ...
  /// Transient layers above the range stay excluded at every limit.
  ///
  /// **Nil is the shipped behaviour and the default.** The layer range excludes
  /// everything below zero because the desktop backdrop fills the screen, which
  /// is a claim about AREA rather than layer, so a coverage bound says it
  /// directly while still admitting the menu-bar strips.
  ///
  /// A full-display backdrop covers exactly 1, so any greater limit admits
  /// everything; the use site reads this as `min(1, limit)`. Validated there
  /// rather than in `init`, since the field is public and settable.
  ...
  /// App prior, then layer prior, then the appearance prior: the most specific
  /// evidence about a window wins.
  ///
  /// Takes owner and layer rather than a `WindowSnapshot` so the offline fit
  /// harness, which works from windows already reduced to coverage, can call it
  /// instead of keeping a second copy of the chain.
...
public enum ExposureModel {
  ...
  /// would read as full-screen window coverage; above the range are transient
  /// pop-up menus and drag images, on screen for a moment and would book a
  /// whole sampling interval.
  ///
  /// **This range does NOT reach the Dock or the menu bar, and never did**
  /// [MEASURED 2026-08-18]. Both live near `-2147483602`, and the window source
  /// applies `.excludeDesktopElements` before this range is consulted. The
  /// measured capture does contain their light, so the two sides of the
  /// comparison are asymmetric there.
  ///
  /// Widening the range is not the fix: the Dock and Wallpaper entries are
  /// full-display backing windows, so admitting them blankets every cell and
  /// deletes the wallpaper term. Only the menu-bar strips are chrome-sized.
  ///
  /// A hidden menu bar is not in the on-screen list, so chrome auto-hide needs
  /// no special case.
  ...
  /// parts of a cell are assumed not to overlap within it until the cell fills.
  public static func modelledGrid(
    ...
    ///
    /// Chrome is admitted by COVERAGE, never by AREA. Chrome windows belong to
    /// every display, so an area test admits another panel's full-screen
    /// backdrop. Coverage is display-local, so a foreign window is exactly 0
    /// and a full-display backdrop exactly 1, and both are refused.
    func admitted(_ window: WindowSnapshot) -> [Double]? {
      ...
      // BELOW the range only. A coverage bound separates the backdrop from
      // chrome, but cannot tell a transient pop-up from real content: both are
      // small.
      ...
      // Capped at 1: a full-display backdrop covers exactly that, and the
      // field is public and settable.
    ...
    case .summedCoverage:
      ...
      for cell in grid.indices {
        ...
        // Clamped because the priors are public and settable; identity at
        // every in-range prior, so the baseline stays bit-identical.
        grid[cell] = min(1, max(0, value))
      }
      return grid

    case .topmostWins:
      // Each cell carries the fraction not yet claimed by a nearer window.
      // With one shared prior the claims telescope to `prior * min(1, sum)`, so
      // this generalises the branch above rather than replacing it.
  ...
  /// Salvaging the good members of a malformed array leaves real cells beside
  /// invented ones in a store that never washes out.
