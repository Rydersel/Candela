...
public enum AudioRoutingPolicy {
  ...
  ///
  /// `actionableDisplayCount` is how many displays the press would ACT on: the
  /// caller's own candidate pool for `mode`, filtered by the same per-display
  /// verdict the key executor applies. It is deliberately not "how many displays
  /// are attached". A watched key is CONSUMED by the tap, so watching it while
  /// nothing can act takes the press away from macOS too and the key does
  /// nothing at all, which is worse than either outcome the rule chooses
  /// between. One display that can act keeps the family armed for the whole rig,
  /// so a press aimed at a display that refuses is still swallowed by that
  /// display rather than sprayed at the others (R1).
  ///
  /// The no-default-output branch sits AHEAD of the count on purpose, keeping
  /// fork parity: with no output device there is no system volume for a released
  /// key to move, so releasing would trade a dead key for a dead key.
