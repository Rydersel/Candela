...
public enum SynthesisSkipReason: Sendable, Equatable {
  /// The per-display opt-in is off.
  case optedOut
  /// No synthesized size was ever stored for this display.
  case nothingStored
  /// A size was stored, and the catalog no longer produces it: the panel
  /// changed, or an existing HiDPI row has taken the stop over.
  case staleDescriptor
  /// Engaging while HDR is on risks the silent HDR drop the revealed-mode
  /// work measured on mode changes.
  case hdrEngaged
  /// The built-in panel is not a synthesis target in v1.
  case builtIn
  /// The stored size is already engaged on this display.
  case alreadyEngaged
  /// The display is in a mirror set the USER built. Engaging would point a
  /// virtual display at a panel already showing another display's
  /// framebuffer, over a set the person asked for.
  case alreadyMirrored
  /// Two slots are reserved for synthesis, and both are in use.
  case noFreeSlot
}
...
public enum SynthesisReapplyPolicy {
  /// - Parameters:
  ///   - optedIn: the per-display opt-in. Part of THIS decision rather
  ...
  ///     what was stored. This is a property of the hardware, so a built-in that
  ...
  ///     asked for.
  ///   - freeSlots: how many of the reserved synthesis slots are unused.
