...
/// "Candela never looked", a different claim from "looked, nothing to report".
public enum ProvenanceSection<Value: Codable & Equatable & Sendable>: Codable, Equatable, Sendable {
  ...
  /// An unrecognized reason fails the whole decode. A fallback "unknown" case
  /// would re-encode to a different raw value and break the record's hash.
