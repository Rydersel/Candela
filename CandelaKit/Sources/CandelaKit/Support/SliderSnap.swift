...
public enum SliderSnap {
  ...
  /// Capture window either side of a stop, in slider units.
  ...
  ///
  /// Clamping happens in BOTH modes and first: the pointer routinely leaves the
  /// capsule on a drag, and downstream controllers take 0…1. Order is observable,
  /// since an overshoot far from every stop (-0.5) would survive a snap-then-clamp.
  ...
  ///
  /// A stepping route has no such thing as "near a stop", so it cannot reuse
  /// `snapped` and must land ON the grid. With `toStops` the grid is `stops`, without
  /// it the multiples of `step`. Either way the result is the next grid point
  ...
  /// cannot do this job: with `stopsWithoutZero` the nearest stop to 0.4 is 0.15
  /// away, far outside `tolerance`, so it hands values between stops back untouched
  /// and a 5% walk-down reaches 0 unimpeded.
  ///
  /// A step never moves against its own direction. A value already below the grid's
  /// lowest point (a volume dragged to 0, or muted) stays put on a decrement rather
  /// than being shoved up to the floor, which would unmute as a side effect of
  /// asking for less.
  ...
  /// Whole-percent text for the readout AND the slider's accessibility value, so the
  /// number a sighted user reads and the one VoiceOver announces come from one call.
