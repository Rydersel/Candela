...
/// prefs itself. D24 got there first, after one hardcoded sentence blamed the
/// display for every grey.
public enum BrightnessSliderPolicy {
  ...
  /// display (WD2's ordering). The verdict is needed for one arm: in pure-DDC
  /// configuration the demotion answers the same full-range software leg
  /// force-software selects, so the path cannot tell those two apart.
