...
struct MirrorApplyTests {
  ...
  /// that does not exist must THROW rather than commit an empty configuration and
  /// report success. Safe on this machine because the staged change names a
  /// nonexistent ID with a NULL master, so even if stage and commit both
  /// succeeded, nothing attached is reconfigured.
  ...
  /// The double every later suite applies mirroring through, so its contract is
  /// pinned here: changes arrive as ONE batch, in order, with the scope asked for.
  ...
  /// Rule 1 as a recorded fact rather than an absence: N changes make ONE entry,
  /// not N. If a refactor loops the transaction per display this is what notices,
  /// and a half-broken mirror set is what the batch shape keeps inexpressible.
  ...
  /// The fake's topology has to FOLLOW the change, or every revert-path test is
  /// tautological: a session that captures, applies, then re-reads `displays()`
  /// sees the capture again and "proves" a revert with nothing to undo.
  ...
  /// Rule 4 on the fake as well. An empty batch that recorded an entry would let
  /// a test assert "mirroring was applied" about a transaction that staged nothing.
  ...
  /// guard BEFORE it reaches anything that can fail, so the fake checks the same
  /// guard before its injection point. Otherwise a session test that set
  /// `failMirroringWith` and drove a revert would enshrine a "revert failed"
  /// branch shipped code cannot reach: `changes(from:to:)` returns `[]` whenever
  /// the live topology already matches the capture, the common case on a revert.
  ...
  /// The fake computes MASTER membership itself; a SLAVE reads
  /// `isInMirrorSet == true` only because `ConfiguredDisplay.init` ORs in
  /// `mirrorsDisplay != kCGNullDirectDisplay`. Nothing else asserts that, so
  /// deleting the OR would leave the fake reporting every slave as outside the
  /// set it is plainly in, suite still green. `setMembers(containing:)` is
  /// asserted from BOTH ends because it is the predicate
  /// `MirrorTopologyPolicy.disengage` breaks a set with.
