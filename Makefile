...
help:
	...
	@echo "  make deploy      markers + freshness + copy to /Applications + launch"
...
markers: release
	...
	hits=$$(strings -a "$(REL_BIN)" | grep -c "$(MARKER)" || true); \
	if [ "$$hits" -ne 0 ]; then \
	  echo "FAIL: $$hits debug marker(s) matching '$(MARKER)' in the Release binary:"; \
	  strings -a "$(REL_BIN)" | grep "$(MARKER)" | sort -u | sed 's/^/        /'; \
	  exit 1; \
	fi; \
	echo "markers: control found ($$ctl hits), no '$(MARKER)' in Release. OK"; \
...
# No re-sign step: cp -R preserves the Developer ID signature, and the old ad-hoc
# re-sign destroyed the identity and its TCC grant (CLAUDE.md §3).
deploy: markers
	@set -euo pipefail; \
	...
	age=$$(( $$(date +%s) - $$(stat -f %m "$(REL_BIN)") )); \
	if [ "$$age" -gt 600 ]; then \
	  echo "REFUSING: $(REL_BIN) is $$age s old; that is not the build you just made."; \
	  exit 1; \
	fi; \
	...
	rm -rf "$(INSTALLED)" && cp -R "$(REL_APP)" "$(INSTALLED)"; \
	codesign --verify --deep --strict "$(INSTALLED)" && echo "    signature intact"; \
	open "$(INSTALLED)"; \
