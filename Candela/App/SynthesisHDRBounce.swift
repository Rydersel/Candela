...
/// the settle window, the optimistic path mirror and the wire-memo
/// invalidation; a window it never heard about leaves the brightness legs
/// writing DDC into a register the display has locked, where each value is
/// ACKed, swallowed, and recorded by the duplicate memo as landed. On a
/// write-only display nothing downstream can tell that from a write that
/// worked, and the slider then sits dead at its last position until the person
/// moves it somewhere else and back.
///
/// Closures rather than a protocol because the wiring is a join the app model
/// owns: a display id has to become a controller, and only `AppModel` holds
/// that table.
struct SynthesisHDRBounce: Sendable {
  ...
  /// No HDR anywhere, so the bounce is a structural no-op. The right seam for
  /// anything that is not about the bounce.
