...
/// Where the checkup flow window goes when a target display is picked (CK16:
...
/// first one AppKit lists, which made a pick read as a random jump.
enum CheckupFlowWindowHost {
  ...
  /// Nil means stay: no target, no other screen, or already off the target.
