...
// they carry no controls and start only once they are on screen and only for a
// visitor who has not asked for less motion. Everything else falls back to the
// poster, including a video whose file is not there.
//
...
export function MediaFrame({ asset, className }: { asset: Asset; className?: string }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ...
      // 0.15, not 0.25: the hero video is taller than a quarter of a 900px
      // viewport leaves room for, so at 0.25 it never started on shorter
      // screens. The observer gates on the fraction of the VIDEO visible,
      // which for a tall element crosses a fixed threshold late.
      { threshold: 0.15 },
    )
    io.observe(el)
  ...
  return (
    <div className="media-shell">
      ...
      <button
        ...
        aria-label={userPaused ? mediaResume : mediaPause}
        onClick={toggle}
      >
        {userPaused ? (
