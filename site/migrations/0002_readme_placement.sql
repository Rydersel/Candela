-- The README's download button is a placement of its own, so its clicks can be
-- told apart from the site's. SQLite cannot widen a CHECK in place, so the
-- table is rebuilt with its rows carried over.
CREATE TABLE unique_actions_next (
  window_key TEXT NOT NULL,
  action TEXT NOT NULL CHECK (action IN ('download', 'github')),
  first_placement TEXT NOT NULL CHECK (first_placement IN ('header', 'hero', 'footer', 'readme')),
